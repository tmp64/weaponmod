...
class PlatformWindows:
    ...
    def get_out_dir(self, path):
        return f'{self.script.paths.build}/{path}/{self.script.build_type}/'
...
class PlatformLinux:
    ...
    def get_out_dir(self, path):
        return f'{self.script.paths.build}/{path}/'
...
class TargetWeaponMod:
    ...
    def get_build_target_names(self):
        return ['weaponmod_amxx']

    def get_file_list(self):
        files = []
        out_dir = self.script.platform.get_out_dir('src')

        files.append(FileToCopy('gamedir/amxmodx', 'amxmodx'))

        files.append(FileToCopy(out_dir + 'weaponmod_amxx' + self.script.platform.get_dll_ext(),
                                'amxmodx/modules/weaponmod_amxx' + self.script.platform.get_dll_ext()))

        if get_platform_type() == 'windows':
            files.append(FileToCopy(out_dir + 'weaponmod_amxx.pdb',
                                    'amxmodx/modules/weaponmod_amxx.pdb'))

        files.append(FileToCopy('README.md', 'amxmodx/README_WeaponMod.md'))
...
class BuildScript:
    ...
    def parse_arguments(self):
        parser = argparse.ArgumentParser(description='Builds production build of WeaponMod, ready for release on '
                                                     'GitHub.')
        ...
        parser.add_argument('--out-dir', action='store',
                            help='output directory (if not set, uses <workdir>/WeaponMod-<version>)')
        ...
        # Set target
        self.build_target = TargetWeaponMod(self)
        ...
        self.out_dir_name = 'WeaponMod-{}-{}-{}-{}'.format(
        ...
        if args.ci:
            with open(os.environ['GITHUB_OUTPUT'], 'a') as f:
                f.write('artifact_name=WeaponMod-{}-{}-{}'.format(
                    self.release_version.replace('+', '-'),
                    get_platform_type(),
                    self.git_hash
                ))
    ...
    def run_cmake(self):
        try:
            ...
            if self.release_version_array[3] == '':
                args.extend(['-DVERSION_TAG=no_tag'])
            else:
                args.extend(['-DVERSION_TAG=' + self.release_version_array[3]])

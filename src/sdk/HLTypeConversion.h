...
{
	return *reinterpret_cast<T*>((reinterpret_cast<int8_t*>(pPrivateData) + offset + (element * sizeof(T))));
}


template <typename T> inline T get_pdata_direct(void *pPrivateData, int offset, int element = 0, int size = 0)
{
	return reinterpret_cast<T>(reinterpret_cast<int8_t*>(pPrivateData) + offset + (element * size));
}
